...
if (isset($_POST['logout'])) {
    session_unset();
    session_destroy();
    header("Location: http://www.leestreetrecords.com");
    exit();
}

session_start();
if (@$_POST['u'] == 'leestreetadmin' && @$_POST['p'] == 'BUilding@wall') { $_SESSION['loggedin'] = true; }
        ...
        for($i=0;$i<count($rows);$i++) {
            ...
            for ($j=0;$j<9;$j++) { // Number of elements should always be 9
                if ($j>=7) { // 7 and 8 need to be strings
                    $rows[$i][$j] = $prefixes[$j].":'".str_replace("'","\'",str_replace("\\","\\\\",$rows[$i][$j]))."'"; // If quote fixing needed
                    //$rows[$i][$j] = $prefixes[$j].":'".$rows[$i][$j]."'"; // Quote fixing not needed
                } else {
...
<!DOCTYPE html>
<html>
    <head><title>Leestreet Band Admin</title>
    <script type="text/javascript" language="javascript">
        var contents = "<?php if ($_SESSION['loggedin']) { echo str_replace(chr(10),'',str_replace(chr(13),'"+String.fromCharCode(10)+"',str_replace('"','\"',str_replace('\\','\\\\',file_get_contents('events.txt'))))); } ?>"; // If quote fixing needed
        //var contents = "<?php if ($_SESSION['loggedin']) { echo str_replace(chr(10),'',str_replace(chr(13),'"+String.fromCharCode(10)+"',file_get_contents('events.txt'))); } ?>"; // Quote fixing not needed
