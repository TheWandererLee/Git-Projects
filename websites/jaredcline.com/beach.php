...
for ($i=0;$i>-1;$i++) {
	if (!file_exists("images/beach/" . $i . ".jpg")) { $last = $i - 1; break; } //Detect last picture
}
if ($last == -1) {
	...
	imagestring($tmp, 8, 8, 90, "Beach", $green);
	imagestring($tmp, 7, 8, 120, "Gallery", $red);
	imagejpeg($tmp, "images/beach/0.jpg");
	imagedestroy($tmp);
	
	header("Location: beach.php");
	exit;
}
if ($_REQUEST['p'] > $last) { header("Location: beach.php?p=".$last); }
if ($_REQUEST['p'] < 0) { header("Location: beach.php?p=0"); }
if (!is_numeric($_REQUEST['p'])) { header("Location: beach.php?p=0"); }

if ($_SESSION['user'] == 'jrizzle') {
	if ($_REQUEST['edit'] == 'left') {
		rename("images/beach/".$_REQUEST['p'].".jpg", "images/beach/temp.jpg");
		rename("images/beach/".strval($_REQUEST['p']-1).".jpg", "images/beach/".$_REQUEST['p'].".jpg");
		rename("images/beach/temp.jpg", "images/beach/".strval($_REQUEST['p']-1).".jpg");
		header("Location: beach.php?p=".strval($_REQUEST['p']-1));
		//exit('<meta http-equiv="refresh" content="0;url=beach.php?p='.strval($_REQUEST['p']-1).'">');
	}
	...
	elseif ($_REQUEST['edit'] == 'delete') {
		unlink("images/beach/".$_REQUEST['p'].".jpg");
		for ($i=$_REQUEST['p']+1;$i<=$last;$i++) {
			rename("images/beach/".$i.".jpg", "images/beach/".strval($i-1).".jpg");
		}
		if ($_REQUEST['p'] == $last) { header("Location: beach.php?p=".strval($_REQUEST['p']-1)); }
	}
	elseif ($_REQUEST['edit'] == 'addleft' || $_REQUEST['edit'] == 'addright') {
		...
		if (empty($fileError)) {
			...
			$saveTo = "images/beach/".$saveNum.".jpg";
...
<title>Jared Cline Photography - Beach Portraits</title>
<?php include("style.php"); ?>
</head>
<body>
<div align="center"><img src="images/beachheader.jpg"></div>
...
<?php
	...
	if ($_SESSION['user'] == 'jrizzle') { $thumbs = 15; } else { $thumbs = getInfo("beachthumbs"); }
	...
	if ($id > 0) { echo '<a href="beach.php?p=' . (string)($id - 1) . '" style="font-size: 36px; text-decoration: none;"><img src="images/back.jpg"></a>'; }
	echo '</td>';
	echo '<td align="center"><a href="image.php?p=beach/' . $id . '&h=768"><img src="image.php?p=beach/' . $id . '&h=320" border="1" style="border-color: #CCCCFF" width="'.getWidth('images/beach/'.$id.'.jpg',320).'" height="320"></a></td>';
	echo '<td width="200" align="left">';
	if ($id < $last) { echo '<a href="beach.php?p=' . (string)($id + 1) . '" style="font-size: 36px; text-decoration: none;"><img src="images/next.jpg"></a>'; }
	echo '</td>';
	echo '</tr>';
	if (!empty($caption[$id])) { echo '<tr><td align="center" colspan="3">' . $caption[$id] . '</td></tr>'; }
	echo '</table>';
	if ($_SESSION['user'] == 'jrizzle') {
		echo '<form name="main" action="beach.php?p='.$id.'" method="post" enctype="multipart/form-data">';
	...
	$side++;
	echo '<td align="right" width="' . (string)round(640 / $thumbs * (100/75)) . '"><a href="beach.php?p=0">&lt;&lt;</a></td>';
	for ($i=$start;$i<=$end;$i++) {
		if ($_SESSION['user'] == 'jrizzle') { $fade = 0; } else { $fade = round(abs($i-$id) / $side * 100, 2); }
		if (file_exists("images/beach/" . $i . ".jpg")) {
			echo '<td><a href="beach.php?p=' . $i . '"><img src="image.php?p=beach/' . $i . '&w=' . (string)round(640 / $thumbs * ((100-$fade)/75)) . '&f=' . $fade . '"';
	...
	echo '<td align="left" width="' . (string)round(640 / $thumbs * (100/75)) . '"><a href="beach.php?p='.$last.'">&gt;&gt;</a></td>';
	echo '</tr></table>';
?>
</body>
</html>