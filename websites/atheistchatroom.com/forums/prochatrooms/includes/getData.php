...
$displayAll = " LIMIT ".($CONFIG['dispLastMess'] + 1);
...
while($i = mysql_fetch_array($tmp)) 
{
	...
	if($showAllUsers == 1)
	{
		...
		$xml .= '<ulastactive>' . ($i['lastActive']) . '</ulastactive>';     
      	$xml .= '<uloginIP>' . ($i['userIP']) . '</uloginIP>';			
		$xml .= '</userlist>';
...
$tmp=mysql_query("
	SELECT id, roomid, roomname, roomowner, roomusers, roomcreated, roomstream, roomstreamer     
...
while($i = mysql_fetch_array($tmp)) 
{
	...
	$xml .= '<roomstream>' . ($i['roomstream']) . '</roomstream>';
	$xml .= '<roomstreamer>' . ($i['roomstreamer']) . '</roomstreamer>';
